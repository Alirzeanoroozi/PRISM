...
def calculate_irmsd_backbone(
    ...
    """Interface backbone iRMSD (Å) between model and reference complexes."""
    warnings.filterwarnings("ignore")

    if len(model_rch) > 1:
        mdl_receptor_chain_combinations = getSymmetricChainsList(model_file, model_rch)
    else:
        mdl_receptor_chain_combinations = [model_rch]

    if len(model_lch) > 1:
        mdl_ligand_chain_combinations = getSymmetricChainsList(model_file, model_lch)
    else:
    ...
    for mdl_rchain in mdl_receptor_chain_combinations:
        for mdl_lchain in mdl_ligand_chain_combinations:
            ref_receptor, model_receptor = getCommonResiduesList(
                ref_file, ref_rch, model_file, mdl_rchain
            )
            ref_ligand, model_ligand = getCommonResiduesList(
                ref_file, ref_lch, model_file, mdl_lchain
            )
            ref_coords, model_coords = getInterfaceResidues(
                ref_receptor, ref_ligand, model_receptor, model_ligand
            )
            if len(ref_coords) == 0 or len(model_coords) == 0:
                curr_irmsd = 100.0
            else:
                curr_irmsd = getRMSD(model_coords, ref_coords)
            if curr_irmsd < least_irmsd:
                least_irmsd = curr_irmsd
    return float(least_irmsd)


def getIRMSD():
    model_file = sys.argv[1]
    model_rch = sys.argv[2]
    model_lch = sys.argv[3]
    ref_file = sys.argv[4]
    ref_rch = sys.argv[5]
    ref_lch = sys.argv[6]
    print("%.3f" % calculate_irmsd_backbone(model_file, model_rch, model_lch, ref_file, ref_rch, ref_lch))
